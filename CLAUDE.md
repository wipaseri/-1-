...
2. **Pages** セクションへ移動
3. **新しいプロジェクト** → **Git に接続** を選択
4. GitHub リポジトリ `wipaseri/tonight-watch` を認可

#### 2. ビルド設定

Cloudflare Pages の設定画面で：

- **フレームワークプリセット**: `なし` (Static site)
- **ビルドコマンド**: 空欄（ビルドステップなし）
- **ビルド出力ディレクトリ**: `/` (ルートディレクトリ)
- **環境変数**: 設定なし

#### 3. 自動デプロイ

- `main` ブランチにプッシュ → 自動的に本番環境へデプロイ
- 個別の commit も自動でプレビューURL生成
...
```javascript
// ❌ 危険：APIキーをソースコードに直接埋め込み
const TMDB_API_KEY = "d0ee623dc5a1cb87863b8947ba776cb6";
```

本番環境では以下の対応を検討：

1. **Cloudflare 環境変数の使用** （推奨）
   - Cloudflare Pages → プロジェクト設定 → 環境変数
   - `TMDB_API_KEY` を設定してから、JavaScript で環境変数経由で読み込む

2. **バックエンド API の実装** （より安全）
   - Cloudflare Workers で TMDb API をプロキシ化
   - クライアント側からは自分のエンドポイント経由でアクセス
...
1. チェックボックスのHTMLを `<div id="providers">` 内に追加
   ```html
   <input type="checkbox" id="p999" value="999" /><label for="p999"
     >新サービス名</label
   >
   ```
...
// APIレスポンスの確認
const url =
  "https://api.themoviedb.org/3/discover/movie?api_key=YOUR_KEY&with_genres=35&language=ja-JP";
fetch(url)
  .then((r) => r.json())
